    ...
    <div>
      ...
      <main className="mx-auto w-[95%] max-w-[1400px] py-4 sm:py-6">
        <section className="rounded-xl border border-slate-700 bg-slate-900 p-4 sm:p-5">
          ...
          <p className="mt-2 text-slate-400">{name}, review your recent Cafe purchases.</p>
        </section>
        ...
        {!loading && orders.length === 0 && !error && (
          <section className="mt-4 rounded-xl border border-slate-700 bg-slate-900 p-4">
            ...
            <Link to="/" className="mt-2 inline-block rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold hover:border-amber-400">
              Browse Menu
            </Link>
          </section>
        )}
        ...
        {!loading && orders.length > 0 && (
          <section className="mt-4 grid gap-3">
            {orders.map((order) => {
              ...
              const itemsLabel = (order.products && order.products.length > 1)
                ...
                >
                  <div>
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      <h3 className="break-all text-lg font-semibold sm:text-xl">{displayOrderNo}</h3>
                      <button
                        type="button"
                    ...
                    <div className="mt-3 grid gap-1 text-sm text-slate-400">
                      {order.products && order.products.length > 0 && order.products.map((product, idx) => (
                        <p key={`${product.id}-${idx}`}>
                          {product.title} x{product.quantity} @ {formatPrice(product.price)}
                        </p>
                      ))}
                    </div>
                  </div>
                  <div className="text-left md:text-right">
                    ...
                    <strong className="mt-1 block text-base">{formatPrice(order.total)}</strong>
                  </div>
                </article>
              );
            })}
